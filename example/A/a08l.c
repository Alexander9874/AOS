...
int main(int argc, char * argv[]) {
    ...
    // The only difference from a08.c is 'l' in lstat
    if (lstat(argv[1], &st) == -1) {
        perror(argv[1]);
        exit(1);
    }
    // It was wrong for my machine
    //
    ...
    printf("Device=%ld Inode=%ld Mode=0%o\n",
           st.st_dev, st.st_ino, st.st_mode);
    printf("Links=%ld UID=%d GID=%d\n",
           st.st_nlink, st.st_uid, st.st_gid);
    printf("TypeDevice=0x%lx | %ld Size=%ld\n",
           st.st_rdev, st.st_rdev, st.st_size);
